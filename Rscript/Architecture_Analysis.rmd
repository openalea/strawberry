...
[**Hint:** put a # before kable when group = F]
...
* ** Comparison of Mean No.Leaves from order 1 between varieties**
...
* ** Comparison of Mean No.flowers from order 2 between varieties**

```{r Comparison of Mean No.flowers from Order 1 onward between varieties}

tab10<-fc_comp_varieties_kruskal_posthoc(data = dat_group_from_Order2,varname = "nb_total_flowers",group = T)


kable(x = tab10,digits = 2,caption = "Mean number with standard deviation and grouping of varieties using ANOVA on rank (Kruskal Wallis and posthoc test) represented by letter from the first-order modules onward")
```
...
kable(x = tab12,digits = 2)
```


```{r Relative frequencies of branch or Extention crown as a function of module orders for each varieties, fig.cap="Relative frequencies of branch or Extention crown as a function of module orders for each varieties"}
...
* ** Comparison between varieties for zeroth-order modules and from first-order module onward**
  ...
  - *Comparison between varieties for zeroth-order module*
```{r}
